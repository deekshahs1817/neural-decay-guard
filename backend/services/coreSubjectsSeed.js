...
// Helper to generate 5 high-quality technical questions per set
function generateQuestionsForSet(courseId, setIndex, setArchetype) {
  const qTypes = ["mcq", "concept", "debugging", "sql_query", "complexity"];
  const questions = [];

  const questionsDatabase = {
    dbms: [
      {
        q: `In ${setArchetype.title}, which property or constraint ensures correct relational integrity?`,
        type: "concept",
        opts: ["Foreign Key Referential Integrity", "Unconstrained NULL values", "Disabling primary keys", "Dropping schema tables"],
        ans: "Foreign Key Referential Integrity",
        exp: "Referential integrity guarantees that a foreign key value must match an existing primary key value in the referenced table."
      },
      {
        q: "What is the result of the SQL query: `SELECT department, COUNT(*) FROM employees GROUP BY department HAVING COUNT(*) > 5;`?",
        type: "sql_query",
        opts: ["Returns departments that have strictly more than 5 employees", "Returns top 5 departments", "Filters out all rows where employee_id > 5", "Syntax Error on HAVING with aggregate"],
        ans: "Returns departments that have strictly more than 5 employees",
        exp: "The HAVING clause filters aggregated groups generated by GROUP BY."
      },
      {
        q: "In a B+ Tree index of order p, where are the actual data record pointers stored?",
        type: "concept",
        opts: ["Exclusively in the leaf nodes (connected as a linked list)", "Only in the root node", "Evenly distributed across all internal nodes", "In separate log files"],
        ans: "Exclusively in the leaf nodes (connected as a linked list)",
        exp: "In a B+ tree, internal nodes only store search keys for navigation; leaf nodes store all data pointers."
      },
      {
        q: "Which normal form requires every non-prime attribute to be non-transitively dependent on every candidate key?",
        type: "concept",
        opts: ["Third Normal Form (3NF)", "First Normal Form (1NF)", "Second Normal Form (2NF)", "Boyce-Codd Normal Form (BCNF)"],
        ans: "Third Normal Form (3NF)",
        exp: "3NF eliminates transitive dependencies (X -> Y -> Z where Z is non-prime)."
      },
      {
        q: "Under Strict Two-Phase Locking (Strict 2PL), when are exclusive (X) locks released by a transaction?",
        type: "complexity",
        opts: ["Only after the transaction commits or aborts", "Immediately after the write operation completes", "During the growing phase", "Whenever requested by another transaction"],
        ans: "Only after the transaction commits or aborts",
        exp: "Strict 2PL holds exclusive locks until end-of-transaction (commit/abort) to prevent cascading rollbacks."
      }
    ],
    os: [
      {
        q: `In ${setArchetype.title}, what is the primary role of the CPU scheduler?`,
        type: "concept",
        opts: ["Selects a ready process from memory and allocates the CPU core to it", "Compiles source code into assembly", "Allocates physical disk sectors", "Controls GPU shader units"],
        ans: "Selects a ready process from memory and allocates the CPU core to it",
        exp: "The short-term CPU scheduler selects from among the processes that are ready to execute and allocates the CPU."
      },
      {
        q: "What condition causes Belady's Anomaly in memory page replacement?",
        type: "concept",
        opts: ["Increasing page frames results in an increased number of page faults under FIFO", "Stack overflow in kernel mode", "Deadlock in multi-core systems", "TLB cache miss rate reaching 100%"],
        ans: "Increasing page frames results in an increased number of page faults under FIFO",
        exp: "Belady's Anomaly occurs in FIFO where allocating more physical frames can paradoxically cause more page faults."
      },
      {
        q: "What is the time complexity of Peterson's Algorithm for mutual exclusion between two processes?",
        type: "complexity",
        opts: ["O(1) entry and exit with busy waiting", "O(N) search", "O(log N)", "O(N^2)"],
        ans: "O(1) entry and exit with busy waiting",
        exp: "Peterson's algorithm executes constant-time flag setting and while-condition evaluation for 2 processes."
      },
      {
        q: "In Banker's Algorithm, if `Need[i] <= Available`, what does the operating system conclude?",
        type: "concept",
        opts: ["Process i can safely finish and release its allocated resources", "A deadlock has occurred immediately", "System enters unsafe state", "Process i must be terminated"],
        ans: "Process i can safely finish and release its allocated resources",
        exp: "If a process's maximum remaining need can be satisfied by current available resources, it is part of a safe sequence."
      },
      {
        q: "What is the difference between a Hard link and a Symbolic (Soft) link in Unix file systems?",
        type: "debugging",
        opts: ["Hard links point directly to the inode number; Soft links contain the pathname of target file", "Hard links work across different file systems; Soft links do not", "Deleting target file preserves Soft link functionality", "Hard links can link directories directly without privileges"],
        ans: "Hard links point directly to the inode number; Soft links contain the pathname of target file",
        exp: "A hard link shares the inode; if the original name is deleted, data remains until link count is 0."
      }
    ]
  };
  ...
  const genericQuestions = [
    {
      q: `What is the core architectural principle governing ${setArchetype.title}?`,
      type: "concept",
      opts: [
        `Optimal structural abstraction and deterministic execution rules in ${setArchetype.focus}`,
        "Randomized unconstrained allocation",
        "Linear O(N!) brute force search",
        "Disregarding boundary safety invariants"
      ],
      ans: `Optimal structural abstraction and deterministic execution rules in ${setArchetype.focus}`,
      exp: `Mastering ${setArchetype.title} requires understanding ${setArchetype.focus}.`
    },
    {
      q: `Which metric is critical when evaluating performance in ${setArchetype.title}?`,
      type: "complexity",
      opts: ["Asymptotic Time & Space Complexity bounds", "Number of comments in code", "File extension name", "Physical monitor resolution"],
      ans: "Asymptotic Time & Space Complexity bounds",
      exp: "Engineering systems optimize for logarithmic, linear, or sub-linear operational bounds."
    },
    {
      q: `Identify the common bug or failure mode in ${setArchetype.title}:`,
      type: "debugging",
      opts: ["Failure to handle boundary/corner conditions and unhandled null state", "Using comments", "Properly structured conditional statements", "Explicit variable declarations"],
      ans: "Failure to handle boundary/corner conditions and unhandled null state",
      exp: "Missing edge case guards and incorrect invariant verification cause system failures."
    },
    {
      q: `In standard technical interviews, what is the best practice when approaching ${setArchetype.title}?`,
      type: "concept",
      opts: [
        "State assumptions clearly, explain tradeoffs, and analyze time/space complexity",
        "Jump straight to unoptimized code without explanation",
        "Assume memory is infinitely fast",
        "Ignore error return codes"
      ],
      ans: "State assumptions clearly, explain tradeoffs, and analyze time/space complexity",
      exp: "FAANG and elite engineering interviews look for structured reasoning and clear communication."
    },
    {
      q: `What is the fundamental invariant of ${setArchetype.title}?`,
      type: "concept",
      opts: [
        "State transitions must preserve consistency and deterministic correctness",
        "Execution must take quadratic time",
        "Data structures must be rebuilt on every access",
        "Interrupts must be disabled permanently"
      ],
      ans: "State transitions must preserve consistency and deterministic correctness",
      exp: "System invariants guarantee that valid pre-conditions always yield valid post-conditions."
    }
  ];
...
function shuffleArray(arr) {
  ...
  const specific = questionsDatabase[courseId];
  const source = (specific && specific.length === 5) ? specific : genericQuestions;

  return source.map(item => ({
    question: item.q,
    type: item.type || "mcq",
    codeSnippet: item.codeSnippet || "",
    options: shuffleArray(item.opts),
...
async function seedCoreSubjects() {
  try {
    const count = await CoreSubjectCourse.countDocuments();
    if (count < 7) {
      console.log("[CSE Core Subjects Seeder] Seeding 7 CSE Core Subject Courses (175 Sets, 875+ Questions)...");
      ...
      console.log(`[CSE Core Subjects Seeder] Successfully seeded all 7 CSE Core Courses with 25 sets each!`);
    } else {
      console.log(`[CSE Core Subjects Seeder] Core Subject Courses already populated (${count} courses).`);
    }
  } catch (err) {
